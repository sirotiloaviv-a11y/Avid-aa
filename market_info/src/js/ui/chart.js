...
export function renderChart(container, bars, { currency = 'USD', tz = 'UTC' } = {}) {
  if (!bars.length) {
  ...
  const draw = () => {
    ...
    bindHover(container, bars, width, currency, tz);
  };
  draw();
...
function scales(bars, width) {
  ...
  let min = Math.min(...bars.map((b) => b.low), ...closes);
  let max = Math.max(...bars.map((b) => b.high), ...closes);
...
function bindHover(container, bars, width, currency, tz) {
  const s = scales(bars, width);
  ...
  const show = (evt) => {
    ...
    tip.innerHTML = `<div>${esc(formatDateTime(b.t, tz))}</div>
      <div><span class="muted">סגירה</span> ${esc(formatPrice(b.close, currency))}</div>
      <div><span class="muted">נפח</span> ${esc(formatCompact(b.volume))}</div>`;
